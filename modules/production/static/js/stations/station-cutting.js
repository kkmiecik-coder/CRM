/**
 * ============================================================================
 * STATION CUTTING - FLAT PRODUCT TILES VERSION
 * ============================================================================
 *
...
(function() {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        console.log('[Cutting] Initializing FLAT PRODUCT TILES station v4.0...');
    ...
    function initializeCuttingStation() {
        ...
        console.log(`[Cutting] Found ${existingCards.length} product tiles`);
        ...
        if (typeof window.initExpandIcons === 'function') {
            ...
            console.log('[Cutting] Expand icons initialized');
        }
        ...
        // Polling postępu składania (synchronizacja z stanowiskiem składania)
    ...
    function updateAssemblyCounters(progress) {
        for (const [productId, data] of Object.entries(progress)) {
            ...
            const currentAssemblyDone = parseInt(card.dataset.quantityDoneAssembly || '0');
            ...
            if (currentAssemblyDone !== newAssemblyDone) {
                card.dataset.quantityDoneAssembly = newAssemblyDone;

                // Znajdź badge-assembled lub stwórz nowy
                let badgeAssembled = card.querySelector('.badge-assembled');
                ...
                if (newAssemblyDone > 0) {
                    ...
                    if (!badgeAssembled && productParams) {
                        ...
                        card.classList.add('has-assembly');
                    ...
                    card.classList.remove('has-assembly');
                }
    ...
    function initializeProductTile(card) {
        ...
        if (!productId) {
            console.warn('[Cutting] Kafelek produktu bez product ID');
            return;
        }

        console.log(`[Cutting] Initializing product tile: ${productId}`);
    ...
    async function handleQuantityButtonClick(card, productId, button) {
        ...
        if (!action) {
            console.warn('[Cutting] Brak akcji na przycisku');
            return;
        }
    ...
    async function sendQuantityUpdate(productId, action, card, previousValue) {
        ...
        const timeoutId = setTimeout(async () => {
            ...
            try {
                ...
                console.error('[Cutting] Błąd aktualizacji ilości:', error);
    ...
    function handleConnectionChange(isOnline) {
        console.log(`[Cutting] Zmiana statusu połączenia: ${isOnline ? 'ONLINE' : 'OFFLINE'}`);
    ...
    function handleCompleteClick(card, productId) {
        console.log(`[Cutting] Kliknięto zakończenie dla produktu: ${productId}`);

        // Sprawdź status połączenia
        if (!window.StationCommon.isOnline()) {
            console.warn('[Cutting] Offline - nie można zakończyć produktu');
    ...
    function startCountdown(card, productId) {
        console.log(`[Cutting] Rozpoczęcie 10-sekundowego odliczania dla ${productId}`);

        const actionContainer = card.querySelector('.order-action');
        if (!actionContainer) {
            console.error('[Cutting] Brak kontenera akcji');
            return;
        }
        ...
        console.log(`[Cutting] Odliczanie rozpoczęte dla ${productId}`);
    }

    function cancelCountdown(card, productId, timerId) {
        console.log(`[Cutting] Odliczanie anulowane dla ${productId}`);
        ...
        const actionContainer = card.querySelector('.order-action');
        if (actionContainer) {
            actionContainer.innerHTML = `<button class="btn-complete" data-product-id="${productId}" data-action="complete" disabled>ZAKOŃCZ WYCIĘCIE</button>`;
        ...
        showToast('info', 'Anulowano wycięcie produktu');
    }
    ...
    async function completeProduct(card, productId) {
        console.log(`[Cutting] Rozpoczęcie completion dla produktu ${productId}`);
        ...
        try {
            ...
            console.log('[Cutting] Produkt zakończony pomyślnie:', result);
            ...
            console.error('[Cutting] Błąd completion produktu:', error);
    ...
    function updateStatsAfterRemoval() {
        ...
        console.log(`[Cutting] Pozostałe kafelki: ${remaining.length}`);
    }
    ...
    function updateHeaderStats() {
        ...
        console.log(`[Cutting] Zaktualizowano statystyki nagłówka: ${allCards.length} produktów, ${totalVolume.toFixed(4)} m³`);
    }
    ...
    function showEmptyState() {
        ...
        ordersList.innerHTML = `
            <div class="empty-state">
                ...
                <h2>Brak produktów do wycięcia</h2>
                <p>Świetna robota! Wszystkie produkty zostały wycięte.</p>
            </div>
        `;

        console.log('[Cutting] Wyświetlanie pustego stanu');
    }
    ...
    function startAutoRefresh(intervalSeconds) {
        ...
        console.log(`[Cutting] Rozpoczęcie auto-refresh co ${intervalSeconds}s`);
    ...
    async function performAutoRefresh() {
        console.log('[Cutting] Wykonywanie auto-refresh...');
        ...
        try {
            ...
            console.log(`[Cutting] Pobrano ${result.data.products.length} produktów`);
            ...
            console.error('[Cutting] Auto-refresh nieudany:', error);
        }
    }
    ...
    function updateHeaderStatsFromAPI(products) {
        ...
        console.log(`[Cutting] Zaktualizowano statystyki z API: ${totalTiles} produktów, ${totalVolume.toFixed(4)} m³`);
    }
    ...
    function smartMergeProducts(newProducts) {
        ...
        newProducts.forEach(product => {
            ...
            if (!existingProductIds.has(productIdStr)) {
                console.log(`[Cutting] Dodawanie nowego produktu: ${product.id}`);
        ...
        existingCards.forEach(card => {
            ...
            if (!apiProductIds.has(productId)) {
                ...
                if (!hasActiveCountdown && !isProcessing) {
                    console.log(`[Cutting] Usuwanie produktu nieobecnego na stanowisku: ${productId}`);
                    card.classList.add('removing');
                    setTimeout(() => {
                        card.remove();
                    ...
                    console.log(`[Cutting] Zachowanie produktu ${productId} - użytkownik nad nim pracuje`);
                }
            }
        });

        console.log('[Cutting] Smart merge zakończony');
    }
    ...
    function createProductTileHTML(product) {
        ...
        // Badge z parametrami
        let paramsHTML = '';
        if (product.quantity_done_assembly > 0) {
            paramsHTML += `<span class="badge badge-assembled">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                    <path d="M19.439 7.85c-.049.322.059.648.289.878l1.568 1.568c.47.47.706 1.087.706 1.704s-.235 1.233-.706 1.704l-1.611 1.611a.98.98 0 0 1-.837.276c-.47-.07-.802-.48-.968-.925a2.501 2.501 0 1 0-3.214 3.214c.446.166.855.497.925.968a.979.979 0 0 1-.276.837l-1.61 1.61a2.404 2.404 0 0 1-1.705.707 2.402 2.402 0 0 1-1.704-.706l-1.568-1.568a1.026 1.026 0 0 0-.877-.29c-.493.074-.84.504-1.02.968a2.5 2.5 0 1 1-3.237-3.237c.464-.18.894-.527.967-1.02a1.026 1.026 0 0 0-.289-.877l-1.568-1.568A2.402 2.402 0 0 1 1.998 12c0-.617.236-1.234.706-1.704L4.315 8.69a.979.979 0 0 1 .837-.276c.47.07.802.48.968.925a2.501 2.501 0 1 0 3.214-3.214c-.446-.166-.855-.497-.925-.968a.979.979 0 0 1 .276-.837l1.61-1.61a2.404 2.404 0 0 1 1.705-.708c.617 0 1.234.236 1.704.706l1.568 1.568c.23.23.556.338.877.29.493-.074.84-.504 1.02-.968a2.5 2.5 0 1 1 3.237 3.237c-.464.18-.894.527-.967 1.02Z"></path>
                </svg>
                <span class="assembled-counter">${product.quantity_done_assembly}/${quantity}</span>
            </span>`;
        }
        ...
        return `
            <div class="order-card${priorityClass}"
                ...
                <div class="order-action">
                    <button class="btn-complete" data-product-id="${product.id}" data-action="complete" disabled>ZAKOŃCZ WYCIĘCIE</button>
                </div>
            </div>
        `;
    }
    ...
    async function fetchTodayM3() {
        try {
            ...
            if (result.success && result.data) {
                ...
                console.log(`[Cutting] Dzisiejsze m³: ${result.data.today_m3}`);
            }

        } catch (error) {
            console.error('[Cutting] Błąd pobierania dzisiejszych m³:', error);
        }
    }
    ...
    window.addEventListener('beforeunload', function() {
        if (state.refreshTimer) {
            clearInterval(state.refreshTimer);
            console.log('[Cutting] Wyczyszczono interwał auto-refresh');
        }

        if (state.countdownTimer) {
            clearInterval(state.countdownTimer);
            console.log('[Cutting] Wyczyszczono timer odliczania');
        }
        ...
        state.activeCountdowns.forEach((countdown, productId) => {
            if (countdown.timerId) {
                ...
                console.log(`[Cutting] Wyczyszczono odliczanie dla ${productId}`);
            }
        });
    ...
    console.log('[Cutting] Moduł załadowany v4.0 (kafelki produktów z przyciskami +/-)');
