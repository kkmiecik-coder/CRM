...
def process_one(now):
    ...
    c = db()
    row = c.execute("SELECT * FROM quote_queue WHERE status='pending' AND next_at<=? ORDER BY id LIMIT 1",
                    (now,)).fetchone()
    c.close()
    if not row:
        return False
    ...
    try:
        ...
        if attempts >= BOT_MAX_ATTEMPTS:
            ...
            c = db(); c.execute("UPDATE quote_queue SET attempts=?, next_at=?, last_error=? WHERE id=?",
