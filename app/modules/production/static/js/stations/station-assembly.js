...
(function() {
    ...
    function smartMergeOrders(newOrders) {
        ...
        console.log(`[Assembly] Smart merge completed: ${addedCount} added, ${removedCount} removed`);
    }
        ...
        if (remainingCards.length === 0) {
            ...
            if (!emptyState) {
                ordersList.innerHTML = `
                    <div class="empty-state">
                        <div class="empty-state-icon">✅</div>
                        <h2>Brak zamówień do montażu</h2>
                        <p>Świetna robota! Wszystkie zamówienia zostały zmontowane.</p>
                    </div>
                `;
            }
        }
