...
# ============================================================================
# ROUTERS - MONITOR PRODUKCJI (TV DISPLAY)
# ============================================================================

@station_bp.route('/monitor')
def production_monitor():
    """
    Monitor produkcji - widok na telewizor pokazujący statusy wszystkich zamówień

    Funkcje:
    - Wyświetla wszystkie zamówienia w dwukolumnowej liście
    - Pokazuje statusy zamówień (bez rozbicia na produkty)
    - Auto-refresh jak inne stanowiska
    - Dark mode only, minimalistyczny design
    - Spakowane tylko z ostatnich 3 dni
    ...
    try:
        ...
        # Oblicz datę 3 dni wstecz dla filtrowania spakowanych
        three_days_ago = datetime.now() - timedelta(days=3)

        # Pobierz wszystkie unikalne zamówienia z agregowanymi danymi
        # Filtr: wszystkie statusy ALBO spakowane z ostatnich 3 dni
        orders_query = db.session.query(
            ...
            func.min(ProductionItem.priority_rank).label('best_priority_rank')
        ).filter(
            or_(
                # Wszystkie zamówienia które NIE są spakowane
                ProductionItem.current_status != 'spakowane',
                # Spakowane tylko z ostatnich 3 dni
                and_(
                    ProductionItem.current_status == 'spakowane',
                    ProductionItem.packaging_completed_at >= three_days_ago
                )
            )
        ).group_by(
            ProductionItem.internal_order_number,
        ...
        order_numbers = [order[0] for order in orders_query if order[0]]

        # Stwórz mapę z danymi agregowanymi
        orders_aggregated = {}
        for order_num, bl_id, total_prod, total_vol, best_rank in orders_query:
            if order_num:
                orders_aggregated[order_num] = {
                    'order_number': order_num,
                    'baselinker_order_id': bl_id,
                    'total_products': total_prod or 0,
                    'total_volume': float(total_vol or 0),
                    'best_priority_rank': best_rank or 999
                }

        # Pobierz szczegółowe dane produktów dla obliczenia statusów
        if order_numbers:
            products_db = ProductionItem.query.filter(
                ProductionItem.internal_order_number.in_(order_numbers)
            ).all()
        else:
            products_db = []

        # Grupuj produkty po zamówieniach i oblicz statusy
        orders_data = {}
        for product in products_db:
            order_num = product.internal_order_number
            ...
            orders_data[order_num]['products'].append(product)

            # Zliczaj statusy
            status = product.current_status
            if status not in orders_data[order_num]['statuses']:
                orders_data[order_num]['statuses'][status] = 0
            orders_data[order_num]['statuses'][status] += 1

        # Przygotuj finalne dane zamówień
        orders_list = []
        for order_num, agg_data in orders_aggregated.items():
            order_details = orders_data.get(order_num, {'products': [], 'statuses': {}})

            # Oblicz wykonane produkty (spakowane)
            completed_count = order_details['statuses'].get('spakowane', 0)
            total_count = agg_data['total_products']

            # Określ dominujący status zamówienia
            order_status = _determine_order_status(order_details['statuses'], total_count)

            orders_list.append({
                'order_number': order_num,
                'baselinker_order_id': agg_data['baselinker_order_id'],
                'total_products': total_count,
                'completed_products': completed_count,
                'total_volume': agg_data['total_volume'],
                'best_priority_rank': agg_data['best_priority_rank'],
                'status': order_status,
                'status_label': _get_status_label(order_status),
                'status_class': _get_status_class(order_status)
            })

        # Sortuj po priority_rank
        orders_list.sort(key=lambda x: x['best_priority_rank'])
        ...
        # Statystyki ogólne
        total_orders = len(orders_list)
        total_products = sum(o['total_products'] for o in orders_list)
        total_volume = sum(o['total_volume'] for o in orders_list)
        completed_orders = sum(1 for o in orders_list if o['status'] == 'spakowane')
...
def _determine_order_status(statuses, total_count):
    """
    Określa dominujący status zamówienia na podstawie statusów produktów

    Args:
        statuses (dict): Słownik {status: count}
        total_count (int): Całkowita liczba produktów

    Returns:
        str: Dominujący status zamówienia
    """
    if not statuses:
        return 'czeka_na_wyciecie'

    # Jeśli wszystkie spakowane
    if statuses.get('spakowane', 0) == total_count:
        return 'spakowane'

    # Jeśli jakiekolwiek anulowane lub wstrzymane
    if statuses.get('anulowane', 0) > 0:
        return 'anulowane'
    if statuses.get('wstrzymane', 0) > 0:
        return 'wstrzymane'

    # Priorytet statusów (od najbardziej zaawansowanego)
    status_priority = [
        'czeka_na_pakowanie',
    ...
    # Znajdź najbardziej zaawansowany status w zamówieniu
...
def _get_status_label(status):
    ...
    Args:
        status (str): Kod statusu

    Returns:
        str: Czytelna etykieta
    """
    labels = {
        'czeka_na_wyciecie': 'Wycinanie',
        'czeka_na_skladanie': 'Składanie',
...
def _get_status_class(status):
    ...
    classes = {
        ...
        'w_realizacji': 'status-in-progress'
    }
